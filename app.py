    ...
    if 'user_hob' in request.args:
        ...
        if hobbie == '':
            return render_template('assignment3_1.html', hobbies_dict=hobbies_dict, social_li=social_li,
                                   alone_li=alone_li, message='think')
        for key in hobbies_dict:
            if hobbie.upper() == key.upper():
                session['user_hobbie'] = hobbie
                session['have_hobbie'] = True
                return render_template('assignment3_1.html', hobbies_dict=hobbies_dict, social_li=social_li, alone_li=alone_li, message='you choose somthing from the list')

            else:
                session['user_hobbie'] = hobbie
                session['have_hobbie'] = True
                return render_template('assignment3_1.html', hobbies_dict=hobbies_dict, social_li=social_li,
                                       alone_li=alone_li, message='new')
